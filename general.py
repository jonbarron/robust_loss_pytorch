...
def lossfun(x, alpha, scale, approximate=False, epsilon=1e-6):
  ...
  Args:
    ...
    alpha: The shape parameter of the loss (\alpha in the paper), where more
      ...
      is 0 at -infinity, infinity, 0, and 2. Must be a tensor or numpy array of
      floats with the same precision as `x`. Varying alpha allows
    ...
    scale: The scale parameter of the loss. When |x| < scale, the loss is an
      ...
      different shape according to alpha. Must be a tensor or numpy array of
  ...
  """
  # Pytorch is apparently broken re: inheriting double precision through as_tensor()?
  if x.dtype == 'float64' or torch.as_tensor(x).dtype == torch.float64:
    float_dtype = torch.float64
  else:
    float_dtype = torch.float32
  x = torch.as_tensor(x, dtype=float_dtype)
  scale = torch.as_tensor(scale, dtype=float_dtype)
  alpha = torch.as_tensor(alpha, dtype=float_dtype)
  ...
  if approximate:
    ...
    machine_epsilon = torch.as_tensor(np.finfo(
        np.float32).eps).type(float_dtype)
