...
class UtilTest(tf.test.TestCase):
  ...
  def testSyuvIsScaledYuv(self):
    ...
    rgb = np.float32(np.random.uniform(size=(32, 32, 3)))
    syuv = np.array(util.rgb_to_syuv(rgb))
    yuv = tf.image.rgb_to_yuv(rgb)
    # Check that the ratio between `syuv` and `yuv` is nearly constant.
    ratio = syuv / yuv
    self.assertAllClose(
        tf.reduce_min(ratio), tf.reduce_max(ratio), atol=1e-4, rtol=1e-4)
  ...
  def testImageDctIsCorrect(self):
    ...
    dct_y = tf.transpose(tf.spectral.dct(im, type=2, norm='ortho'), [0, 2, 1])
    dct_x = tf.transpose(
        tf.spectral.dct(dct_y, type=2, norm='ortho'), [0, 2, 1])
    im_dct_true = dct_x
